...
namespace Magewirephp\Magewire\Features\SupportMagewireCompiling\View\Precompiler;

use Magewirephp\Magewire\Features\SupportMagewireCompiling\View\Precompiler;

abstract class DomNodePrecompiler extends Precompiler
{
    /**
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all()).']); ?>';
    ...
    {
        return $this->stringStartsWith($value, ['"', '\''])
            ? substr($value, 1, -1)
            : $value;
    }

    private function stringStartsWith(string $haystack, array $needles): bool
    {
