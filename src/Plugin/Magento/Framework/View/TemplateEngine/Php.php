...
class Php
{
        ...
        if ($block instanceof DataObject) {
            ...
            if ($magewire instanceof Component) {
                ...
                if (! isset($this->components[$magewire->id()])) {
                    ...
                    $renderComponentTemplate = trigger('magewire:component-template:render', $block, $filename, $dictionary, $magewire);

                    $result = ['block' => $block, 'filename' => $filename, 'dictionary' => $dictionary];
                    $result = $renderComponentTemplate($result);
    ...
    public function afterRender(Subject $subject, string $html): string
    {
        ...
        if ($latest && ( $latest['component'] ?? null ) instanceof Component) {
            ...
            $finish = trigger('magewire:component-template:rendered', $latest);

            $html = $finish($html);
        }

        trigger('magento:template:rendered', $html);
        return $html;
    }
}
