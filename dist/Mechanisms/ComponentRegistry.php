...
class ComponentRegistry extends Mechanism
{
    ...
    protected function getNameAndClass($nameComponentOrClass)
    {
        ...
        if (!is_subclass_of($class, Component::class)) {
            throw new ComponentNotFoundException("Unable to find component: [{$nameOrClass}]");
        }
    ...
    protected function generateClassFromName($name)
    {
        ...
        $class = collect(str($name)->explode('.'))->map(fn($segment) => (string) str($segment)->studly())->join('\\');
    ...
    {
        $namespace = str_replace(['/', '\\'], '.', trim(trim(config('livewire.class_namespace')), '\\'));
        $class = str_replace(['/', '\\'], '.', trim(trim($class, '/'), '\\'));
        $namespace = collect(explode('.', $namespace))->map(fn($i) => \Illuminate\Support\Str::kebab($i))->implode('.');
        $fullName = str(collect(explode('.', $class))->map(fn($i) => \Illuminate\Support\Str::kebab($i))->implode('.'));
