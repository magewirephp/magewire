...
use Magewirephp\Magewire\Wireable;
use Magewirephp\Magewire\Exceptions\MissingRulesException;
...
trait HandlesValidation
{
    ...
    {
        return store($this)->set('errorBag', $bag instanceof MessageBag ? $bag : new MessageBag($bag));
    }
    public function resetErrorBag($field = null)
    {
        ...
        $this->setErrorBag($this->errorBagExcept($fields));
    }
    ...
    {
        $fields = (array) $field;
        return new MessageBag(collect($this->getErrorBag())->reject(function ($messages, $messageKey) use ($fields) {
    ...
    public function getRules()
    {
        ...
        } else if (property_exists($this, 'rules')) {
            $rulesFromComponent = $this->rules;
        }
        $rulesFromOutside = array_merge_recursive(...array_map(fn($i) => value($i), $this->rulesFromOutside));
    ...
    protected function getMessages()
    {
        ...
        $messagesFromOutside = array_merge(...array_map(fn($i) => value($i), $this->messagesFromOutside));
    ...
    protected function getValidationAttributes()
    {
        ...
        $validationAttributesFromOutside = array_merge(...array_map(fn($i) => value($i), $this->validationAttributesFromOutside));
    ...
    public function rulesForModel($name)
    {
        ...
        return collect($this->getRules())->filter(function ($value, $key) use ($name) {
    ...
    public function hasRuleFor($dotNotatedProperty)
    {
        ...
        return collect($this->getRules())->keys()->map(function ($key) {
    ...
    public function ruleWithNumbersReplacedByStars($dotNotatedProperty)
    {
        ...
        return (string) str($dotNotatedProperty)->replaceMatches('/(?<=(\.))\d+\./', '*.')->replaceMatches('/\.\d+$/', '.*');
    }
    ...
    {
        collect($rules)->keys()->each(function ($ruleKey) use ($data) {
            throw_unless(array_key_exists(Utils::beforeFirstDot($ruleKey), $data), new \Exception('No property found for validation: [' . $ruleKey . ']'));
        });
    }
    ...
    public function validateOnly($field, $rules = null, $messages = [], $attributes = [], $dataOverrides = [])
    {
        ...
        if ($this->isRootComponent() && ($form = $this->all()[$property] ?? false) instanceof Form) {
        ...
        $rulesForField = collect($rules)->filter(function ($value, $rule) use ($field) {
        ...
        try {
            ...
            invade($e->validator)->messages = $messages->merge($this->errorBagExcept($ruleKeysForField));
            throw $e;
        }
