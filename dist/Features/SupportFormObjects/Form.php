...
use Magewirephp\Magewire\Features\SupportValidation\HandlesValidation;
...
class Form implements Arrayable
{
    ...
    public function validateOnly($field, $rules = null, $messages = [], $attributes = [], $dataOverrides = [])
    {
        try {
            ...
            invade($e->validator)->messages = $this->prefixErrorBag(invade($e->validator)->messages)->merge($this->getComponent()->getErrorBag());
            invade($e->validator)->failedRules = $this->prefixArray(invade($e->validator)->failedRules);
            throw $e;
        }
    }
